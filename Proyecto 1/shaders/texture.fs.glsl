...
void main () {
    ...
    /*
    vec3 sampledMaterialKd = vec3(texture(material.texture0, fragmentTextureCoordinates));

    vec3 N = normalize(fragmentNormal);
    vec3 L = normalize(vec3(fviewMatrix * allLights[0].position) - fragmentPosition);
    float LdotN = dot(L, N);

    vec3 ambient = sampledMaterialKd * allLights[0].color * 0.05;
    vec3 diffuse = sampledMaterialKd * allLights[0].color * max(LdotN, 0.0);

    vec3 outgoingLight = ambient + diffuse;
